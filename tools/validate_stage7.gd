...
func _ready()->void:
	...
	if game==null or story==null or access==null or scheduler==null or saves==null:errors.append("Stage 7 autoload missing")
		...
		var old:Dictionary=saves.migrate_save({"save_version":5,"saved_at_unix":0,"game":{},"story":{},"memory_puzzle":{},"access_mask":{}});if int(old.get("save_version",0))!=7 or bool((old.get("scheduler",{}) as Dictionary).get("event_started",true)) or bool((old.get("integer_range",{}) as Dictionary).get("stage_started",true)):errors.append("P v5 migration unsafe")
