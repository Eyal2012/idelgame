...
func _ready()->void:
	...
	panel=PanelContainer.new();panel.name="IntegerRegisterPanel";panel.custom_minimum_size=Vector2(280,190);panel.mouse_filter=Control.MOUSE_FILTER_IGNORE;add_child(panel)
	...
	label=Label.new();label.name="RegisterText";label.add_theme_font_size_override("font_size",12);label.add_theme_color_override("font_color",Color("bdeff4"));label.mouse_filter=Control.MOUSE_FILTER_IGNORE;panel.add_child(label)
...
func _refresh()->void:
	...
	label.text="INTEGER REGISTER\n\nTYPE   SIGNED INT32\nMIN    -2,147,483,648\nMAX     %s\nCURRENT %s\nUTILIZATION %.2f%%\nHEADROOM %s"%[_fmt(range.get_int32_max()),_fmt(game.get_currency()),range.get_capacity_percent(),_fmt(range.get_headroom())];_position()
